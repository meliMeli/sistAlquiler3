...
function Paginador(myDiv, nroPagina, numRegistros, funcionSeleccion, nroRegistrosPorPagina) {
    this.simple = false;
    this.myDiv = myDiv;
    ...
    this.activaCombo = false;
}
function Paginador(myDiv, nroPagina, numRegistros, funcionSeleccion, nroRegistrosPorPagina, activarPaginacion) {
    this.simple = false;
    this.myDiv = myDiv;
    this.nroPagina = nroPagina;
    this.numRegistros = numRegistros;
    this.funcionSeleccion = funcionSeleccion;
    this.nroRegistrosPorPagina = nroRegistrosPorPagina;
    this.activaCombo = activarPaginacion;
}
...
function fnPaginadorMini(paginador) {
    console.log('numRegistros:'+paginador.numRegistros); 
    $("#" + paginador.myDiv).paging(paginador.numRegistros, {
        perpage: paginador.nroRegistrosPorPagina, //nro de paginas.
        page: paginador.nroPagina,
        format: '[< - nncn - >]',
        onFormat: function(type) {
            switch (type) {
                ...
                case 'block':
                    if (!this.active)
                        return '<li><span>' + this.value + '</span></li>';
                    else if (this.value != this.page)
                        return '<li ><a href="#' + this.value + '" onclick="' + paginador.funcionSeleccion + '(' + this.value + ')">' + this.value + '</a></li>';
                    return '<li ><span class="opcion_activa">' + this.value + '</span></li>';

                case 'right':
                    if (!this.active) {
                        return "";
                    }
                    return '<li><a href="#' + this.value + '"<' + this.value + ' onclick="' + paginador.funcionSeleccion + '(' + this.value + ')" ></a></li>';
                case 'left':
                ...
                case 'next':
                    ...
                    if (this.active) {
                        return '<li><a href="#' + this.value + '" class="next" onclick="' + paginador.funcionSeleccion + '(' + this.value + ')">Siguiente &raquo;</a></li>';
                    }
                    return '<li class="active" ><span>Sig &raquo;</span></li>';

                case 'prev':

                    if (this.active) {
                        return '<li><a href="#' + this.value + '" class="prev" onclick="' + paginador.funcionSeleccion + '(' + this.value + ')">&laquo; Anterior</a></li>';
                    }
                    return '<li class="active" ><span>&laquo; Ant</span></li>';

                case 'first':

                    if (this.active) {
                        return '<li><a href="#' + this.value + '" class="first" onclick="' + paginador.funcionSeleccion + '(' + this.value + ')">|&lt;</a></li>';
                    }
                    return '<li class="active" ><span>|&lt;</span></li>';

                case 'last':
                    var DOM = '';
                    if (this.active) {
                        DOM = '<li ><a href="#' + this.value + ' class="prev" onclick="' + paginador.funcionSeleccion + '(' + this.value + ')">&gt;|</a></li>'
                    } else {
                        DOM = '<li  class="active" ><span>&gt;|</span></li>';
                    }
                    if (paginador.activaCombo) {
                        DOM = DOM + ' <li  class="active"> <div><select id="cmbCantPag"  style="margin: 0 0;">';
//                        DOM = DOM + '<option value="5">5</option>';
                        DOM = DOM + '<option value="10">10</option>';
                        DOM = DOM + '<option value="20">20</option>';
                        DOM = DOM + '<option value="50">50</option>';
                        DOM = DOM + ' </select> </div></li>  ';
                    }
                    ;

                    DOM = DOM + ' <li class="right" ><span class="active " style="border-left-width: 1px;">Total de registros : ' + paginador.numRegistros + '</span></li> ';
