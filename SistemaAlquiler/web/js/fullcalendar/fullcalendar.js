...
(function($, undefined) {
    ...
    var defaults = {
        ...
        // time formats
        titleFormat: {
            year: 'Año yyyy', // 1.- Añadido para hacer el yearView
            month: 'MMMM yyyy',
            week: "MMM d[ yyyy]{ '&#8212;'[ MMM] d yyyy}",
            day: 'dddd, MMM d, yyyy'
        },
        columnFormat: {
            year: 'MMMM', //2.-Añadido para yearView
            month: 'ddd',
            week: 'ddd M/d',
            day: 'dddd M/d'
        },
        ...
        dayNamesShortest: ['D', 'L', 'M', 'M', 'J', 'V', 'S'], //3.-Añadido para la vista Anual
        buttonText: {
            prev: '&nbsp;&#9668;&nbsp;',
            next: '&nbsp;&#9658;&nbsp;',
            prevYear: '&nbsp;&lt;&lt;&nbsp;',
    ...
    function BasicYearView(element, calendar, viewName) {
        ...
        disableTextSelection(element.addClass('fc-grid table table-striped')); /**/
        ...
        function buildSkeleton(maxRowCnt, showNumbers) {
            ...
            var localWeekNames = ['D', 'L', 'M', 'M', 'J', 'V', 'S'];
            var localMonthNames = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
            ...
            for (var mi = 0; mi < 12; mi++) {
                ...
                s += "<td class='fc-year-monthly-td'  style='font-weight:bold; padding-left:1px;'>";
...
//            bodyRows.filter('.fc-year-have-event-correcto').removeClass('fc-year-have-event-correcto');/*PENDIENTE DE MANTENIMIENTO*/
        }
            ...
            try {
                ...
                for (var i = 0; i < segs.length; i++) {
                    ...
                    while (sd.getTime() < segs[i].end.getTime()) {
                        if (segs[i].event.data !== null) {
                            if (segs[i].event.data.estado === 1) {
                                rowsTd.filter('.fc-day-' + formatDate(segs[i].start, 'yyyy-MM-dd')).addClass('fc-year-have-event-correcto');
                            } else {
                                rowsTd.filter('.fc-day-' + formatDate(segs[i].start, 'yyyy-MM-dd')).addClass('fc-year-have-event');
                            }
                            rowsTd.filter('.fc-day-' + formatDate(segs[i].start, 'yyyy-MM-dd')).attr('id', segs[i].event.data.id);
                            rowsTd.filter('.fc-day-' + formatDate(segs[i].start, 'yyyy-MM-dd')).attr('title', segs[i].event.title);
                        } else {
                            rowsTd.filter('.fc-day-' + formatDate(segs[i].start, 'yyyy-MM-dd')).addClass('fc-year-have-event');
                        }
                        addDays(sd, 1);
